...
## Load Data ##
global_region_tensor = np.load('data/processed/spatial_temporal/global_region_tensor_scaled_sg5.npy', allow_pickle=True)
col_names = load_column_names()
locations = load_locations()
target_regions = locations['region_id']
...
TAR_INDEX = col_names.index(TAR_VAR)
SEED = 112358

# Data Params
MAX_SG = 5 #maximum spatial granularity of local region from input data
...
SPATIAL_FEATURES = NUM_FEATURES-TEMPORAL_FEATURES-3 #remove date field, lat, lon, 4*time cyclic fields
...
RGN_ID_EMB_DIM = 8 # from 357 indian lat-lon regions
...
FC1_UNITS = 128
FC2_UNITS = 128
MODEL_NAME = f"{PRED_TAR}_sp-tm_sg{str(SG)}_seq{str(SEQ_LEN)}_stp{str(STEP)}_emb{str(RGN_ID_EMB_DIM)}"
...
def train_spatial_temporal_model(model, dataset_generator, opt='adam', epochs=EPOCHS, steps_per_epoch=STEPS_PER_EPOCH,
    ...
    filepath = f"models/model_saves/{PRED_TAR}.hdf5"
    ...
    # Include tensorboard
    if include_tb:
        tensorboard_cb = tf.keras.callbacks.TensorBoard(f"models/logs")
    ...
    model.compile(loss='mean_absolute_error', optimizer=optimizers[opt],
                  metrics=[MeanAbsoluteError(), RootMeanSquaredError(), Huber()])
